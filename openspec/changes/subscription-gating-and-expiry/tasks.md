...
- [x] 1.1 抽出 `isServiceable(subscription, now)`：`ACTIVE` / 试用中 / 宽限内 → true
- [x] 1.2 宽限截止派生为 `currentPeriodEnd + 2d`，不落库（D3）
- [x] 1.3 单测覆盖六个 Shopify 状态 × 试用中/外 × 宽限内/外的组合
- [x] 1.4 单测：同一 `currentPeriodEnd` 反复判定，宽限截止不后移

## 2. 计费周期（改 `QuotaService`）

- [x] 2.1 `periodStart` 锚点改为 `trialEnds`，试用期内不推进周期
- [x] 2.2 移除「为失效订阅把周期推进到包含现在那一期」的行为，只对可服务订阅推进
- [x] 2.3 `planCodeFor` 加状态筛选，不再拿作废订阅的档位当额度依据
- [x] 2.4 单测：一年未续的订阅不再每 30 天重置额度
- [x] 2.5 单测：同店多条订阅时取可服务的那条
...
- [x] 4.1 订阅闸门置于 `assertWithinQuota` **之前**（D1）
- [x] 4.2 被拦下时顾客话术不暴露商家套餐/欠费/用量，会话转 `pending`
- [x] 4.3 单测：订阅失效 + 额度也耗尽时，拦截原因记为订阅失效，且零上游调用
- [x] 4.4 商家端显示停服原因与恢复入口，与「额度用尽」区分开

## 5. 到期提醒

- [x] 5.1 按 0.3 选定的方案实现按天触发
- [x] 5.2 D-3 / D-2 / D-1 各一次；不足三天时不补发已错过的档位
...
- [x] 6.1 `app_subscriptions/update` 到达且状态转为可服务时，闸门随即放行
- [x] 6.2 单测：`FROZEN` → 收到 `ACTIVE` 的 webhook → 无需人工即恢复
- [x] 6.3 单测：webhook 迟到但仍在宽限窗口内时服务不中断

## 7. 收尾

- [x] 7.1 `pnpm test`（含 `pnpm spec`）全绿
