...
| Chunk | 状态 | 说明 |
|---|---|---|
| 0 跑阶段 A | ✅ 完成 | 四屏 spec（108/131/131/143 元素）+ baseline。自比噪声地板 **0**，四屏 S 级 |
| 1 令牌扩充 | ✅ 完成 | 令牌 18 → 50，DS-8 零交集，DS-9 三态 33 个名一致 |
| 2 阶段 A 重建 | ✅ 完成 | 44 处刻意偏离；axis1 不过项全部是中文字宽及连带位移，颜色/字体/内边距零损伤 |
| 3 阶段 B 移植 | ✅ 完成 | 四屏 React 化，class 逐字保留；三处对 skill 的明确偏离已记录 |
| 4 阶段 C 复核 | ⚠️ **未达标** | 遮罩后 6.81%–17.76%，门是 ≤0.36%。红=0（无结构漂移） |
| 5 收尾 | ✅ 完成 | 登录页补齐；DESIGN.md §2 记录参照关系与偏离 |

**本文件尚未降为历史快照** —— Chunk 4 未达标，计划未完成。

### 阶段 C 未达标的已知构成

1. **输入本身矛盾（不可消除）**：四屏 Stitch 稿的壳互不一致——侧栏底色 `#f6fbf6` vs
   `#d6dbd7`、宽度 200px vs 256px、`audit_log` 多一条深色顶栏与英文导航。
   已按事实 6 以 `expiry_queue` 为准；`account_detail` 的 13.86% 壳层差异即由此而来。
2. **剩余布局尺度差（可消除，未做）**：内容区 5.78%–14.87%。文本遮罩只降 1–2 个点，
   说明不是字形噪声。需逐元素对 `spec.json` 调间距与行高。

### 复核途中定位并修掉的三个真问题

- `max-w-container-max` 是未迁移的 spacing 令牌，类名不生效导致内容区撑满全宽 → `max-w-[1280px]`
- 侧栏 212px → 200px
- **目标图缺陷**：Stitch 静态稿 body 背景不铺满视口高，露出 html 白底，
  `account_detail` 因此虚高到 84.93%；补满高后降到 5.84%

### 下一步（若要推到 A 级）

对每屏跑 `verify.js --spec .stitch/spec/<page>.target.spec.json --rebuilt <项目侧快照>`，
按 `topFixes` 逐条改间距/行高。或先决 `TBD-5`，把阶段 C 接进 `pnpm spec` 门禁。
