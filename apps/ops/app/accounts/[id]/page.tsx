...
const CELL = 'font-data-mono text-data-mono border-ink flex items-center border-r px-3';
const HEAD =
  'font-label-caps text-label-caps text-on-surface-variant border-ink flex items-center border-r px-3 py-2 uppercase';
    ...
    <AuthGate>
      <OpsShell active="accounts" padded={false}>
        <div className="max-w-[1280px] mx-auto flex w-full flex-1 flex-col gap-6 overflow-auto p-[16px] md:p-6 lg:p-8">
          ...
          {/* 账号头卡 */}
          <div className="bg-card-surface border-ink relative border">
            <div className="absolute right-[16px] top-[16px] z-10">
              <button
                type="button"
                onClick={() => shops[0] && impersonate(shops[0].shopDomain)}
                disabled={!shops.length}
                className="bg-ink text-on-primary border-ink font-label-caps text-label-caps hover:bg-card-surface hover:text-ink flex items-center gap-2 border px-4 py-2 uppercase transition-colors disabled:opacity-40"
              >
                <LogIn className="h-4 w-4" aria-hidden="true" />
                代登录
              </button>
            </div>
            <div className="border-ink flex flex-col items-start gap-6 border-b p-6 md:flex-row md:items-center md:p-8">
              <div className="bg-surface border-ink flex h-20 w-20 shrink-0 items-center justify-center border">
                <User className="text-ink h-10 w-10" aria-hidden="true" />
              </div>
              <div>
                <h2 className="font-headline-lg text-headline-lg text-ink m-0 mb-1">
                  {account?.email ?? '…'}
                </h2>
                <div className="font-data-mono text-data-mono text-on-surface-variant flex flex-wrap items-center gap-2">
                  ...
                  <span className="bg-ink mx-2 h-1 w-1 rounded-full" />
                  <span>ID: {account?.id ?? '—'}</span>
                  <span className="bg-surface text-on-surface-variant font-label-caps border-ink ml-2 border px-2 py-0.5 text-[9px] uppercase">
                    {account?.role ?? '—'}
                  </span>
                </div>
              </div>
            ...
            <div className="divide-ink border-ink bg-surface-container-low grid grid-cols-2 divide-x divide-y border-t md:grid-cols-4 md:divide-y-0">
              <div className="p-4">
                <div className={STAT_LABEL}>名下店铺</div>
                <div className="font-headline-md text-headline-md text-ink">{shops.length}</div>
              </div>
              <div className="p-4">
                <div className={STAT_LABEL}>安装最早</div>
                <div className="font-data-mono text-headline-sm text-ink">{fmtDate(earliest)}</div>
              </div>
              <div className="p-4">
                <div className={STAT_LABEL}>最近登录</div>
                <div className="font-data-mono text-headline-sm text-ink">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            {/* 名下店铺 */}
            <div className="bg-card-surface border-ink flex flex-col border lg:col-span-2">
              <div className="border-ink bg-surface flex items-center justify-between border-b p-2">
                <h3 className="font-label-caps text-label-caps text-ink m-0 flex items-center gap-2 px-2 uppercase">
            ...
            {/* 最近操作 */}
            <div className="bg-card-surface border-ink flex flex-col border">
              <div className="border-ink bg-surface flex items-center justify-between border-b p-2">
                <h3 className="font-label-caps text-label-caps text-ink m-0 flex items-center gap-2 px-2 uppercase">
